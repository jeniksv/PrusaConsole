...
command_tree::command_tree() : _root() {}

command_tree::command_tree(std::shared_ptr<command> _root) : _root(_root) {}
...
std::vector<std::string> command_tree::get_complete_options(const std::string& path_tree){
	...
	return node->is_leaf() ? get_path_options(next_prefix) : get_composite_command_options(std::dynamic_pointer_cast<composite_command>(node), next_prefix);
}
...
void command_tree::complete_composite_command(const std::shared_ptr<composite_command> node, std::string& current, const std::string& prefix){
...
void command_tree::complete_path(std::string& current, const std::string& prefix){
	...
	for(const auto& entry : std::filesystem::directory_iterator(directory)){
		...
		options.insert(directory == std::filesystem::current_path() ? std::filesystem::relative(entry.path()).string() : entry.path().string());
	}
